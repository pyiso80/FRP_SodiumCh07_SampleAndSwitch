...
public class dynamic {
        ...
        static class CreateCharacters
        {
                                 ...
                                 Stream<Set<Integer>> sDestroy)
                {
                        ...
                        HomoZombicus z = new HomoZombicus(
                                                initState.nextID,
                                                new Point(36, 332),
                                                time,
                                                sTick,
                                                scene);
                        initState = initState.add(
                                        z.character,
                                        z.sBite,
                                        fallDownHole(
                                                initState.nextID,
                        ...
                        Stream<Lambda1<State, State>> sAdd =
                                periodicTimer(time, sTick, 6.0).map(
                                        u ->
                                        st ->
                                        {
                                                BitableHomoSapiens h =
                                                        new BitableHomoSapiens(
                                                                world, st.nextID,
                                                                center, time, sTick,
                                                                sBite, scene
                                                        );
                                                return st.add(h.character,
                                                        h.sBite,
                                                        fallDownHole(st.nextID,
                                                                sTick,
                                                                h.character,
                                                                world));
                                        });
                        ...
                        state.loop(
                                sChange.snapshot(state, (f, st) -> f.apply(st))
                                        .hold(initState));
                        Cell<Cell<List<Character>>> cchars =
                                state.map(st -> sequence(st.chars.values()));
                        this.scene = Cell.switchC(cchars);
                        Cell<Stream<Set<Integer>>> csBite =
                                state.map(st -> Helper.mergeToSet(st.sBites.values()));
                        this.sBite = Cell.switchS(csBite);
                        Cell<Stream<Set<Integer>>> csDestroy =
                                state.map(st -> {
                                        System.out.println("s Destroys");
                                        return Helper.mergeToSet(st.sDestroys.values());
                                });
                        /*
                        Cell<Stream<Set<Integer>>> csBite =
                                sTick.snapshot(state, (u, st) -> {
                                        //System.out.println(st.sBites.values());
                                        return Helper.mergeToSet(st.sBites.values());
                                }).hold(sBite);
                        this.sBite = Cell.switchS(csBite);
                        Cell<Stream<Set<Integer>>> csDestroy =
                                sTick.snapshot(state, (u, st) -> {
                                        //System.out.println(st.sDestroys.values());
                                        return Helper.mergeToSet(st.sDestroys.values());
                                }).hold(sDestroy);
                         */
